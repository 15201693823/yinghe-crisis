...
class MiningScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.image(width / 2, height / 2, 'bg_mining').setDisplaySize(width, height).setDepth(0);
        const vig = this.add.graphics();
        vig.fillStyle(0x000000, 0.2); vig.fillRect(0, 0, width, 38);

        this.add.text(width / 2, 48, 'D层 · 矿业营地', {
            fontSize: '16px', fill: '#d4a574', fontFamily: 'Microsoft YaHei', fontStyle: 'bold',
            backgroundColor: '#0a0a1eCC', padding: { x: 16, y: 6 }
        }).setOrigin(0.5).setDepth(5);
        this.add.text(width / 2, 70, '矿工的生活与抗争', {
            fontSize: '9px', fill: '#e0c0a0', fontFamily: 'Microsoft YaHei',
        ...
        for (let i = 0; i < 12; i++) {
            const dust = this.add.circle(
                ...
                Phaser.Math.Between(1, 3), 0xd4a574, Math.random() * 0.2 + 0.05
            ).setDepth(7);
            this.tweens.add({
                targets: dust, y: dust.y - Phaser.Math.Between(20, 60), alpha: 0,
        ...
        for (const [id, pos] of Object.entries(npcPositions)) {
            ...
            this.tweens.add({ targets: s, y: pos.y - 3, duration: 2200 + Math.random() * 800, yoyo: true, repeat: -1, ease: 'Sine.easeInOut' });
        ...
        this.areaPanel = createAreaInfoPanel(this, '矿业营地', '矿工的生活与抗争', [
            { name: '老赵', faction: 'miner' },
            { name: '小梅', faction: 'miner' }
        ], 0xd4a574);
        ...
        this.dialogueContainer = createEnhancedDialogue(this, 'mine-chat', 0xd4a574);
        this.handleSendDialogue = async () => {
            const el = document.getElementById('mine-chat');
